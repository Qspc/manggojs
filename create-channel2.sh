...
# notes buat port apakah harus diubah?
...
#     export CORE_PEER_LOCALMSPID="PengumpulMSPMSP"
...
createChannel(){
    ...
    -f ./artifacts/channel2/${CHANNEL_NAME}.tx --outputBlock ./channel-artifacts/${CHANNEL_NAME}.block \
...
updateAnchorPeers(){
    setGlobalsForPeer0Penangkar
    peer channel update -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com -c $CHANNEL_NAME -f ./artifacts/channel2/${CORE_PEER_LOCALMSPID}anchors_CHANNEL_NAME.tx --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA
